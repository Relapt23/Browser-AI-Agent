...
- Видео на YouTube воспроизводятся автоматически при открытии страницы /watch. Не нужно кликать play. Но убедись, что нет блокирующих элементов (cookie-баннер, модалки).
- Не считай задачу выполненной, если URL не изменился после клика. Проверяй URL в page state — он должен соответствовать ожидаемому результату.
- Если нужно открыть видео и URL содержит /results (страница поиска) — видео ещё не открыто, нужно кликнуть по нему.
"""
